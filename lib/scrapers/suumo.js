...
async function fetchAreaPageOnce(prefSlug) {
  ...
  if (!abt || cities.length === 0) return null;
  return { ar: abt[1], bs: abt[2], ta: abt[3], cities };
}

// 「市区郡にチェック」パネル（/tochi/<pref>/city/）から地域コード(ar/bs/ta)と市区郡一覧を取得。
// 一時的な通信不調でページが取得できない（または市区郡一覧が空になる）ことがあるため、
// 失敗時は少し待って一度だけ再試行する。
async function fetchAreaPage(prefSlug) {
  let area = await fetchAreaPageOnce(prefSlug);
  if (!area) {
    await new Promise(r => setTimeout(r, 1200));
    area = await fetchAreaPageOnce(prefSlug);
  }
  return area;
}
...
async function searchSuumo(criteria, _context) {
  ...
  try {
    if (criteria.city) {
      ...
      if (!area) {
        warning = '市区郡情報の取得に失敗したため、市区郡・町村を絞り込めませんでした。少し時間をおいて再度お試しください。';
      } else {
        const city = area.cities.find(c => c.name.includes(criteria.city) || criteria.city.includes(c.name));
        if (!city) {
          warning = `「${criteria.city}」に一致する市区郡が見つかりませんでした。`;
        } else {
